...
- **Inter-container traffic** uses compose service names (`app`, `media-node`, `srs`) over the isolated `ingest` bridge network — never host ports/localhost. Host ports are published only for outside consumers: the browser (app :31954) and OBS (RTMP :1935). SRS's FLV (38080, configurable via the node's `SRS_HTTP_PORT`) and API (1985) ports stay inside the network: the media-node advertises its FLV base (`SRS_FLV_BASE`, e.g. `http://srs:38080`) at registration, and the app's same-origin playback proxy pulls per-session from the hosting node — viewers never reach SRS directly. `PUBLIC_HOST` is the OBS/browser-visible IP only.
