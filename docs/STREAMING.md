...
- **Playback is WebRTC ONLY:** browsers watch via WHEP — the SDP exchange rides the app's admin-gated same-origin proxy (/api/streams/whep, so media can never start without an authenticated session), and the MEDIA flows directly browser ↔ the hosting node's SRS over UDP (ICE candidate = SRS_RTC_CANDIDATE, derived from the OBS authority host; default port 38000 via SRS_UDP_PORT, published on the srs service — UDP cannot be path-routed through an HTTP proxy, and the published port MUST equal the rtc listen because the SDP candidate carries it). There is no FLV path for browsers; server-side frame capture still pulls FLV internally via SRS_FLV_BASE. A wall-clock watchdog surfaces connection failures (ICE can hang forever on blackholed UDP); ICE_SERVERS (app env, JSON array or comma list) can add STUN/TURN for the browser side — empty by default because SRS's public candidate is directly reachable.
